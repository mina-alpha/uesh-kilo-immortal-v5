...
//! Providers: Ankr, PublicNode, Official, 1RPC
//!
//! Features:
...
fn default_endpoints() -> Vec<RpcEndpoint> {
    vec![
        ...
        RpcEndpoint::new("https://rpc.ankr.com/eth", "ethereum", "ankr"),
        RpcEndpoint::new("https://ethereum-rpc.publicnode.com", "ethereum", "publicnode"),
        RpcEndpoint::new("https://1rpc.io/eth", "ethereum", "1rpc"),

        // Base (3 endpoints)
        RpcEndpoint::new("https://mainnet.base.org", "base", "official"),
        RpcEndpoint::new("https://rpc.ankr.com/base", "base", "ankr"),
        RpcEndpoint::new("https://base-rpc.publicnode.com", "base", "publicnode"),

        // Arbitrum (3 endpoints)
        RpcEndpoint::new("https://arb1.arbitrum.io/rpc", "arbitrum", "official"),
        RpcEndpoint::new("https://rpc.ankr.com/arbitrum", "arbitrum", "ankr"),
        RpcEndpoint::new("https://arbitrum-one-rpc.publicnode.com", "arbitrum", "publicnode"),

        // Polygon (3 endpoints)
        RpcEndpoint::new("https://polygon-rpc.com", "polygon", "official"),
        RpcEndpoint::new("https://rpc.ankr.com/polygon", "polygon", "ankr"),
        RpcEndpoint::new("https://polygon-bor-rpc.publicnode.com", "polygon", "publicnode"),
    ]
}
...
impl RpcRotator {
    ...
    pub async fn get_rpc_url(&self, chain: &str) -> Result<String, RpcError> {
        ...
        if chain_eps.is_empty() {
            warn!("[RPC] All {} endpoints exhausted.", chain);
        ...
        for attempt in 0..max_retries {
            match self.get_rpc_url(chain).await {
                ...
                Err(RpcError::AllEndpointsExhausted(_)) => {
                    warn!(
                        "[RPC] All endpoints exhausted on attempt {}.",
                        attempt + 1
                    );
    ...
    /// Health check all endpoints via eth_blockNumber.
...
mod tests {
    ...
    fn test_endpoints_per_chain() {
        ...
        for chain in &["ethereum", "base", "arbitrum", "polygon"] {
            ...
            assert_eq!(count, 3, "Chain {} should have 3 endpoints", chain);
        }
    }
}
